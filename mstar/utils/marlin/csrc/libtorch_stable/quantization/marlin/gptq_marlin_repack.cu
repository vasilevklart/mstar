// GPTQ-Marlin weight-repack CUDA op, vendored from vLLM (Apache-2.0).
//
// Source: vllm-project/vllm
//   csrc/libtorch_stable/quantization/marlin/gptq_marlin_repack.cu
// The __global__ ``gptq_marlin_repack_kernel`` is copied VERBATIM; only the host
// wrapper + op registration are rewritten from vLLM's ``torch::stable`` ABI to
// the classic ``torch::Tensor`` + ``TORCH_LIBRARY`` ABI mstar's vendored ops use
// (see utils/fused_moe/csrc/moe_align_block_size.cu). W4A16 only: the ``is_a_8bit``
// (W4A8) template arm is dropped (fixed to false), so no fp8/int8 activation path.
// Registered under ``_mstar_marlin_C`` so it never collides with a real vLLM ``_C``.
//
...
// W4A16 only: is_a_8bit is fixed to false, so only the has_perm ∈ {false, true}
// arms are instantiated (act-order not exercised by Kimi, but kept for reuse).
...
torch::Tensor gptq_marlin_repack(torch::Tensor b_q_weight, torch::Tensor perm,
  ...
  TORCH_CHECK(perm.scalar_type() == torch::kInt, "perm type is not kInt");

  const at::cuda::OptionalCUDAGuard device_guard(device_of(b_q_weight));
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  const int device_index = b_q_weight.get_device();
...
// The op schemas for the whole ``_mstar_marlin_C`` namespace are declared by the
// single TORCH_LIBRARY block in marlin_moe.cu (one per namespace per extension);
// this file only provides the repack impl.
